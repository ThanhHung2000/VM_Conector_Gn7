...
class PCBCheckerApp:
    ...
    def detect_connector_pose(self):
        ...
        list_cos = []
        last_frame = None
        # 3. QUÉT TÌM GÓC XOAY LỆCH (ANGLE OFFSET)
        ...
        for _ in range(5):
            ...
            if circles is not None:
                ...
                # 2. Tính Sobel Gradient tìm góc
                grad_x = cv2.Sobel(blurred, cv2.CV_64F, 1, 0, ksize=3)
                grad_y = cv2.Sobel(blurred, cv2.CV_64F, 0, 1, ksize=3)
                magnitude = cv2.magnitude(grad_x, grad_y)
                magnitude = cv2.normalize(magnitude, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

                # 3. Quét 360 độ tìm góc lệch của frame hiện tại
                found_key_angle = 0
                img_h, img_w = frame.shape[:2]

                # 1. Quét 360 độ và lưu lại bán kính r của TẤT CẢ các góc
                scan_step = 1  # Quét mịn 1 độ để có dải điểm đầy đủ
                angles_r = {}
                ...
                for a in range(0, 360, scan_step):
                    ...
                    angles_r[a] = max_r_at_angle

                # 2. Lọc bỏ nhiễu: Chỉ giữ lại các góc có r đủ lớn (gần với r cực đại)
                max_r_all = max(angles_r.values()) if angles_r else 0
                threshold_r = max_r_all - 5  # Cho phép dung sai 5px so với đỉnh cao nhất

                # Lấy các góc thỏa mãn điều kiện độ cao
                valid_angles = [a for a, r in angles_r.items() if r >= threshold_r]

                # --- ĐOẠN KHẮC PHỤC (Thay thế cho Phần 3) ---
                if valid_angles:
                    # 1. Tách valid_angles thành các cụm góc liên tục
                    clusters = []
                    current_cluster = [valid_angles[0]]

                    for i in range(1, len(valid_angles)):
                        # Nếu 2 góc liên tiếp cách nhau <= 3 độ -> Cùng 1 cụm (1 tai)
                        if valid_angles[i] - valid_angles[i-1] <= 3:
                            current_cluster.append(valid_angles[i])
                        else:
                            clusters.append(current_cluster)
                            current_cluster = [valid_angles[i]]
                    clusters.append(current_cluster)

                    # Dồn cụm nếu vắt qua mốc 0 - 360 độ (Ví dụ cụm 358-359 độ và cụm 0-2 độ)
                    if len(clusters) > 1 and (360 - clusters[-1][-1] + clusters[0][0]) <= 3:
                        clusters[0] = clusters[-1] + clusters[0]
                        clusters.pop()

                    # 2. Chọn cụm có số lượng điểm nhiều nhất (Tai rõ ràng nhất)
                    best_cluster = max(clusters, key=len)

                    # 3. Lấy trung vị của cụm tốt nhất này
                    mid_idx = len(best_cluster) // 2
                    found_key_angle = best_cluster[mid_idx] % 360
                else:
                    found_key_angle = 0

                # 4. Tính angle_offset như cũ
                ...
                # Lưu dữ liệu frame này vào mảng
                list_cx.append(float(cx))
                list_cy.append(float(cy))
                list_r_in.append(float(r_in))
                
                # Đổi góc sang Sin/Cos để tính trung bình vector góc (chống nhiễu góc bù)
                rad_offset = math.radians(angle_offset)
                list_sin.append(math.sin(rad_offset))
                list_cos.append(math.cos(rad_offset))

            self.root.update()
    # --- 2. TÍNH GIÁ TRỊ TRUNG BÌNH CỦA 10 FRAMES ---
        if len(list_cx) == 0:
            print("[WARNING] Không tìm thấy Connector trong 10 frames!")
    ...
    def process_image(self, img,cx=None, cy=None, angle_offset=0.0,r_input=None):
        ...
        sector_angle = 30 # góc quét là 30 vừa đủ chứa tai
