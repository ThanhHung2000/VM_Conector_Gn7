...
# --- BƯỚC NÂNG CẤP CÔNG NGHIỆP: TỰ ĐỘNG TÌM GÓC XOAY CONNECTOR ---
    ...
    def inspect_current_frame(self):
        """Kích hoạt kiểm tra trên Frame hiện tại và tự động phóng to Connector trên cả 2 ảnh"""
        if not hasattr(self, 'current_frame') or self.current_frame is None:
            return
        # Tạo bản sao độc lập truyền vào hàm xử lý
        clean_frame = self.current_frame.copy()
        # Chạy thuật toán xử lý ảnh
        processed_img, edge_img, status, ear_data = self.process_image(clean_frame)

        if processed_img is not None:
            # 1. Cập nhật nhãn trạng thái OK / NG
            if "NG" in status or "KHÔNG" in status:
                self.lbl_result.config(text=status, fg="red")
            else:
                self.lbl_result.config(text=status, fg="green")

            # 2. CẮT VÙNG PHÓNG TO CONNECTOR (PHÓNG TO CẢ ẢNH MÀU VÀ BẢN ĐỒ ĐEN TRẮNG)
            gray = cv2.cvtColor(processed_img, cv2.COLOR_BGR2GRAY)
            h_img, w_img = processed_img.shape[:2]
            
            # Tìm lại tâm để lấy vùng Crop bao quanh Connector
            circles = cv2.HoughCircles(
            ...
            if circles is not None:
                ...
                # Lề mở rộng xung quanh connector (tỷ lệ theo bán kính r)
                crop_margin = 520
                # Ép kiểu int thuần của Python để tính tọa độ crop
                cy_int = int(cy)
                cx_int = int(cx)
                margin = int(crop_margin)
                # Giới hạn tọa độ trong phạm vi bức ảnh
                x1, x2 = max(0, cx_int - margin), min(w_img, cx_int + margin)
                y1, y2 = max(0, cy_int - margin), min(h_img, cy_int + margin)
                
                # Cắt lấy riêng vùng Connector phóng to
                main_crop = processed_img[y1:y2, x1:x2]
                edge_crop = edge_img[y1:y2, x1:x2] if edge_img is not None else None
            else:
                # Nếu không tìm thấy tâm thì hiển thị toàn bộ ảnh
                main_crop = processed_img
                edge_crop = edge_img

            # 3. TIỂU CHUẨN HÓA KÍCH THƯỚC HIỂN THỊ TRÁNH BIẾN DẠNG (Aspect Ratio 1:1)
            # Vì vùng Crop là hình vuông bao quanh Connector nên phóng hiển thị kích thước 480x480 sẽ không bị méo.
            display_size = (520, 520)

            # --- BỔ SUNG ĐOẠN CHỐNG CRASH BẢO VỆ CV2.CVTCOLOR ---
            if main_crop is None or main_crop.size == 0:
                print("[WARNING] Ảnh main_crop bị rỗng! Bỏ qua frame này.")
                return  # Bỏ qua không vẽ nữa để không bị sập app

            # Hiển thị Ảnh Kết Quả Đo (Phóng to Connector)
            ...
            # Hiển thị Bản Đồ Cạnh Đen Trắng (Phóng to Connector)
            ...
            # 4. Cập nhật 6 ảnh chi tiết tai
            for i, (crop_img, area, ear_status, color) in enumerate(ear_data):
                ...
                self.ear_labels[i].config(text=f"Tai {i+1}: {area} ({ear_status})", fg=fg_color)
